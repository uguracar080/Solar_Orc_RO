...
% (YENİ) PTC field -> HX arası sirkülasyon pompası
circ = struct();
if cfg.use_circ_pump
    ...
    Th_HX_in_K = circ.Tout_K;   % HX hot giriş sıcaklığı pompa sonrası
else
    circ.ok = false;
...
% ------------------------------
% (ENERGY) Circulation pump power bookkeeping
% ------------------------------
if cfg.use_circ_pump && isfield(circ,'ok') && circ.ok
    out.circ_pump_ok    = true;
    out.Wcirc_elec_kW   = circ.Welec_W/1000;   % [kW] electrical consumption
    out.Wcirc_shaft_kW  = circ.Wshaft_W/1000;  % [kW] to fluid (optional)
else
...
% ------------------------------
% (ENERGY) System net power including auxiliaries (circ pump)
% ------------------------------
Wcirc_kW = 0.0;
if exist('circ','var') && isstruct(circ) && isfield(circ,'ok') && circ.ok
    Wcirc_kW = circ.Welec_W/1000;
end

out.Wnet_sys_kW = out.Wnet_kW - Wcirc_kW;   % [kW] system net (ORC net - circulation pump)
...
if cfg.do_exergy && isfield(out,'exergy') && isfield(out.exergy,'ok') && out.exergy.ok
    ...
    % -------------------------------------------------------------
    % (3.1) Circulation Pump (PTC->HX) exergy terms
    % -------------------------------------------------------------
    if cfg.use_circ_pump && isfield(circ,'ok') && circ.ok
        out.exergy.Wcirc_elec_kW = circ.Welec_W/1000;  % [kW]
        out.exergy.ExD_circ_kW   = max(0, circ.ExD_kW); % [kW] destruction
    else
...
if cfg.show_tables && isfield(out,'exergy') && isfield(out.exergy,'ok') && out.exergy.ok % Tablo gösterme koşulu + exergy çıktısı var mı + başarılı mı
    ...
    ExBreak = table( ...                                                                % Exergy sonuç tablosu oluştur
        ...
        out.exergy.ExD_circ_kW, ...                                                     % [kW] Eşanjör sirkülasyon pompası exergy yıkımı
        ...
        'VariableNames', { ...                                                          % Kolon isimlerini ekrana yazdır
            ...
            'ExD_ORC_total', ...
            'Ex_rej_cond', ...
            'ExD_PTC','ExD_HX','ExD_circ','ExD_System', ...                         % Toplamlar
